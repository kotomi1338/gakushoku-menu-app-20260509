...
git switch main
git pull
git switch -c feature/a-display-price
```
...
```bash
git add .
git commit -m "[Aさん] 一覧画面に金額を表示する変更"
git push origin feature/a-display-price
```
...
git switch main
git pull
git switch -c feature/b-display-good
```
...
```bash
git add .
git commit -m "[Bさん] いいね機能の見た目実装"
git push origin feature/b-display-good
```
...
```bash
git pull origin feature/b-display-good
```
